...
public class CassandraHiveMetaStore implements RawStore {
  ...
  public void createDatabase(Database database) throws InvalidObjectException, MetaException {
    ...
    metaStorePersister.save(database.metaDataMap, database, SchemaManagerService.DATABASES_ROW_KEY);
  }
  ...
  public List<String> getDatabases(String databaseNamePattern) throws MetaException {
    ...
    List<TBase> databases = metaStorePersister.find(new Database(),
            SchemaManagerService.DATABASES_ROW_KEY, databaseNamePattern, 100);
  ...
  public boolean dropDatabase(String databaseName) throws NoSuchObjectException,
    ...
    metaStorePersister.remove(database, SchemaManagerService.DATABASES_ROW_KEY);
    return true;
  }
  ...
  public boolean addRole(String roleName, String ownerName)
    ...
    metaStorePersister.save(role.metaDataMap, role, SchemaManagerService.META_DB_ROW_KEY);
    return true;
  }
  ...
  public Role getRole(String roleName) throws NoSuchObjectException {
    ...
    role.setRoleName(roleName);
    try {
      metaStorePersister.load(role, SchemaManagerService.META_DB_ROW_KEY);
    } catch (NotFoundException nfe) {
  ...
  public boolean createType(Type type) {
    metaStorePersister.save(type.metaDataMap, type, SchemaManagerService.META_DB_ROW_KEY);
    return true;
  }
  ...
  public Type getType(String type) {
    ...
    t.setName(type);
    try {
      metaStorePersister.load(t, SchemaManagerService.META_DB_ROW_KEY);
    } catch (NotFoundException e) {
      return null;
    }
    return t;
  ...
  public boolean dropType(String type) {
    Type t = new Type();
    t.setName(type);
    metaStorePersister.remove(t, SchemaManagerService.META_DB_ROW_KEY);
    return true;
  }
